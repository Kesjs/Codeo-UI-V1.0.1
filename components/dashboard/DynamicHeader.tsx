...
import {
  Sparkles, Zap, History, Layout, Cpu, Folder, Palette, Activity, Users,
  Settings, Bell, User, Menu, ChevronDown, Search, X, Check
} from 'lucide-react'
    ...
    >
      <div className="px-4 lg:px-6 py-3">
        <div className="flex flex-col space-y-3">
          {/* Première ligne : Section active, recherche et actions utilisateur */}
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="hidden sm:flex flex-col items-center min-w-0 flex-shrink-0 max-w-[100px]">
              <button
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className="px-3 py-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
              >
                <span className="font-medium text-sm text-slate-700 dark:text-slate-300 truncate">{currentSection?.title}</span>
              </button>
              <div className={`h-0.5 w-full bg-codeo-green transition-all duration-200 ${pathname === currentSection?.path ? 'opacity-100' : 'opacity-0'}`}></div>
            </div>

            {/* Barre de recherche */}
            <div className="relative flex-1 min-w-0" ref={searchResultsRef}>
            ...
            <div className="flex items-center gap-1 sm:gap-3 flex-shrink-0">
              ...
              <div className="hidden md:flex items-center gap-2 px-3 py-1.5 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                ...
                <div className="text-sm">
                  <div className="font-semibold text-slate-900 dark:text-white">
                    {credits.total === Infinity ? 'Illimités' : `${credits.used}/${credits.total}`}
                  </div>
                  ...
                  {isNotificationMenuOpen && (
                    ...
                    >
                      ...
                      <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                        ...
                        <button className="text-xs text-codeo-green hover:text-codeo-green/70">Tout marquer comme lu</button>
                      </div>
                      ...
                      <div className="max-h-96 overflow-y-auto">
                        <div className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors border-b border-slate-100 dark:border-slate-700">
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1">
                              <div className="text-sm font-medium text-slate-900 dark:text-white">Scan IA terminé</div>
                              <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">Votre projet "Dashboard Pro" a été scanné avec succès</div>
                              <div className="text-xs text-slate-400 mt-1">Il y a 2 minutes</div>
                            </div>
                        ...
                        <div className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors border-b border-slate-100 dark:border-slate-700">
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1">
                              <div className="text-sm font-medium text-slate-900 dark:text-white">Nouveau membre</div>
                              <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">Marie a rejoint votre équipe Business</div>
                              <div className="text-xs text-slate-400 mt-1">Il y a 15 minutes</div>
                            </div>
                            <div className="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0 mt-2"></div>
                          </div>
                        </div>
                        ...
                        <div className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                          <div className="flex gap-3">
                            ...
                            <div className="flex-1">
                              <div className="text-sm font-medium text-slate-900 dark:text-white">API activée</div>
                              <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">Vos webhooks sont maintenant actifs</div>
                              <div className="text-xs text-slate-400 mt-1">Il y a 1 heure</div>
                            </div>
                          </div>
                        </div>
                      </div>
              ...
              <div className="relative" ref={profileMenuRef}>
                ...
                >
                  ...
                  <span className="hidden sm:block font-medium text-sm text-slate-700 dark:text-slate-300">Jean Dupont</span>
                  <ChevronDown className={`hidden sm:block w-4 h-4 text-slate-400 transition-transform ${isProfileMenuOpen ? 'rotate-180' : ''}`} />
                </button>
                    ...
                    >
                      ...
                      <div className="px-4 py-3 border-b border-slate-200 dark:border-slate-700">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-medium text-slate-900 dark:text-white">Jean Dupont</div>
                            <div className="text-xs text-slate-500 dark:text-slate-400">jean.dupont@email.com</div>
                          </div>
                        </div>
                      </div>
